...
| **Users** | Owners cannot change privilege, verification, identity, or legacy mirror fields on update (`isAdmin`, `role`, `status`, `verifiedAt`, `candidateId`, `legacyData`, etc.); those writes use **Admin SDK** server actions (`src/app/applicant/verification-actions.ts`). Self-updates and creates are limited by **key allowlists** (`userSelfUpdateAffectedKeysAllowlist`, `usersCreateKeysValid` in `firestore.rules`). Admins retain full read/write. |
...
| 2026-03-24 | Firestore: key allowlists + types on users/candidateIds/applicants/pendingVerifications/deletion/bridge/interview/audit self-create | Deployed rules; see `docs/firestore-field-type-validation.md` |
...
- [`docs/firestore-field-type-validation.md`](docs/firestore-field-type-validation.md) — Field/key allowlists and type checks in `firestore.rules` (unexpected-field hardening).
